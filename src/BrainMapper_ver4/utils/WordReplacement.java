...
public class WordReplacement extends JDialog {
    ...
    public static final WordReplacement INSTANCE = new WordReplacement();
    ...
    private void replace() {
        ...
        String str = Node.getTextarea().getSelectedText();
        if (str == null) {
            return;
        }
        if (str.equals(searchingWord)
                ...
                .isSelected())) {
            Node.getTextarea().replaceSelection(replacingWord);
        }
    ...
    private void replaceAll() {
        ...
        String replacingWord = replaceTextField.getText();
        String str = Node.getTextarea().getText();
        if (UpperLowerCheckBox.isSelected()) {
            ...
            // J2SE 5.0以降用コード
            // Node.setText(Pattern.compile(searchingWord, Pattern.LITERAL)
            // .matcher(str).replaceAll(replacingWord));
            Node.getTextarea().setText(str.replaceAll("\\Q" + searchingWord,
                    replacingWord));
        } else {
            ...
            // J2SE 5.0以降用コード
            // Node.setText(Pattern.compile(searchingWord,
            // Pattern.LITERAL | Pattern.CASE_INSENSITIVE).matcher(str)
            // .replaceAll(replacingWord));
            Node.getTextarea().setText(Pattern.compile("\\Q" + searchingWord,
    ...
    private void search(boolean isSelected) {
        ...
        if (isSelected) {
            int num = Node.getTextarea().getCaretPosition();
            ...
            Node.getTextarea().select(num, num + word.length());
        } else {
            ...
            int num = Node.getTextarea().getSelectionStart() - 1;
