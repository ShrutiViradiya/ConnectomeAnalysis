...
#Rscript CalcMesOfBinaryAndUndirectedNet_v5.R ./wd_NMLMET_vs_NMLVAL_deep+surf/binary_undirected/ 1
...
for(s in start:num_subject){
    ...
    cat("ecount(g)=", ecount(g), "\n")
    
#
# エッジ本数＝1の場合の計算エラーを回避するためのロジック
#
    if(ecount(g)==1){
        cat("エッジ数1の場合の特別分岐", "\n")
        cat("# エッジ本数１本のネットワークの示す各種値が正しいか後日確認する必要あり（2017.06.11）", "\n")
        ...
        closen <- rep(0, vcount(g))
        val <- c(val,closen)
        
        # eigenvecotr centrality
        eigen <- rep(0, vcount(g))
        val <- c(val,eigen)
        ...
        # Hub score
        hubscore <- rep(0, vcount(g))
        val <- c(val,hubscore)
        
        # Burt's constraint
        constraint <- rep(0, vcount(g))
        val <- c(val,constraint)
        
        # Coreness
        coreness <- rep(0, vcount(g))
        val <- c(val,coreness)
        
        # Cocitation
        cocitation <- rep(0, vcount(g))
        val <- c(val,cocitation)
        
        # nodal global efficiency
        nodal_global_efficiency <- rep(0, vcount(g))
        val <- c(val,nodal_global_efficiency)
        
        # nodal local efficiency
        nodal_local_efficiency <- rep(0, vcount(g))
        val <- c(val,nodal_local_efficiency)
